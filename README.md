...
- локализация: наборы ключей совпадают, подстановки не разъезжаются, язык переключается
...
Интерфейс на русском и английском. По умолчанию берётся язык системы, вручную
переключается в настройках — без перезапуска.

Строки лежат словарями в `Sources/GuitarKeys/Localization.swift`, а не в каталогах
`.lproj`: приложение собирается вызовом `swiftc` без Xcode, и ресурсные каталоги
пришлось бы подкладывать в бандл руками. Полноту перевода проверяет тест — он
сверяет наборы ключей и совпадение подстановок в обоих языках.

## Автор

Vladislav Kovalskyi

- GitHub — [@vladislavkovalskyi](https://github.com/vladislavkovalskyi)
- Telegram — [@yxuxo](https://t.me/yxuxo)
