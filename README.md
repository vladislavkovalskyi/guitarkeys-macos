...
Второй экран приложения — переключатель слева в шапке. Проект собирается из тактов,
в каждом восемь восьмых.

![Студия](docs/studio.png)

Выбираете кисть — удар вниз, вверх, глушёный, отдельная струна или нота по ладу — и
расставляете по сетке. Повторный тап по той же ячейке стирает. Такт можно показать
двумя способами: строкой боя или табулатурой на шесть струн.

**Табулатура.** В режиме табов у такта шесть строк, по одной на струну, и в ячейку
ставится номер лада. Так переносят песню с табов: лад задан явно и не зависит от
аккорда такта. Кисть «# лад» выбирает нужный лад степпером, а тяга по ячейке
вверх-вниз перебирает лады — удобно подбирать на слух.

**Сила удара.** Ползунок задаёт громкость новых событий, а тяга по ячейке вверх-вниз
меняет силу уже поставленных. Высота заливки в ячейке и есть громкость, так что
динамика читается с одного взгляда.

**Порядок тактов** меняется перетаскиванием за шапку такта — за ячейки тянуть нельзя,
они заняты рисованием.

Проект сохраняется в `~/Music/GuitarKeys/Проекты`, сводится в файл кнопкой «Свести»:
M4A, WAV, AIFF или CAF на выбор. Сведение идёт офлайн и заметно быстрее реального
времени — восемь секунд музыки собираются примерно за десятую долю секунды.
